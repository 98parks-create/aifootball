...
- 2026-04-17: 타겟 Re-ID 전면 재설계 — detector.py 완전 재작성: (1) 상체+반바지 2-region HSV 지문, (2) ID 스위치 즉시 감지(코사인 유사도 0.30 이하 시 재탐색), (3) 소실 즉시(4초 대기 없이) 외모+위치 결합 재탐색, (4) EMA bbox 크기 패널티 추가, (5) 소실 길이에 따라 탐색반경 동적 확대
- 2026-04-19: 전체 점검 & 이메일 발송 활성화 — .env 파일 생성 (MAIL_USER/MAIL_PASS/MAIL_FROM/BASE_URL), Gmail SMTP 발신 계정 98parks@gmail.com 설정 완료. 분석 완료 시 로그인한 사용자 이메일로 결과 링크 자동 발송 정상 작동.
- 2026-04-19: 패스 성공률 + 캘리브레이션 자동화 구현 — analyzer.py: _check_pass_result 메서드 추가 (팀원 볼 수신 여부 1.5초 내 추적), PASS/LONG PASS/THROUGH BALL 이벤트에 pass_result 필드 추가, extract_combined_highlights 반환에 pass_stats 포함. app.py: _auto_detect_field_corners 함수 추가 (HSV 녹색 감지 → 볼록껍질 4각 근사), /api/auto_calibrate/<session_id> 엔드포인트 추가. index.html: 🔍 경기장 자동 감지 버튼 + 초기화 버튼, 패스성공률 stat-badge 추가, autoCalibrate()/resetCalib() JS 함수 추가.
